...
function getPersonFromAPI(data) {


    let dobOpt = {year: 'numeric', month: 'long', day: 'numeric'};

    return {
        name: data.name.first + " " + data.name.last,
        bio: quote[Math.floor(Math.random() * quote.length)].text,
        dob: new Date(data.dob.date).toLocaleDateString(undefined, dobOpt),
        profileUrl: "",
        profilePicUrl: data.picture.medium,
    }
...
class Search extends React.Component {
    ...
    componentDidMount() {
        ...
        fetch("https://randomuser.me/api/?inc=name,dob,gender,picture&results=10")
            .then(resp => resp.json())
            .then(result => {
                ...
                result.results.forEach(person => {
                    persons.push(getPersonFromAPI(person));
                })
        ...
        fetch("https://randomuser.me/api/?inc=name,dob,gender,picture&results=10")
            ...
            .then(result => {
                ...
                this.setState((prevState) => {
                    ...
                    return {
                        friends: [...prevState.friends, ...persons],
                        isLoading: false
                    }
